...
# proxy_scrape_go
プロキシ 서버를 사용하여 Colly, GOquery, Selenium으로 Webスクレイピング을 수행하는 방법을 시연하는 기본 API sergo 프로그램입니다
...
이 프로젝트는 [Web Scraping in Go](https://brightdata.co.kr/blog/how-tos/web-scraping-go)에서 プロキシ 서버를 설정하는 방법을 시연합니다. プロキシ는 Webスクレイピング 중 자신의 IPアドレス를 사용하여 디지털 신원을 보호하고, IP 차단 및 지역 차단(geoblocking)을 우회하는 데 도움이 됩니다.
...
3. 의존성을 설치합니다:
    ...
    ```
## Usage
이 프로젝트는 Go에서 プロキシ 서버를 사용하여 Webスクレイピング을 수행하는 방법을 시연합니다. 다음 라이브러리를 활용합니다:
- [Colly](https://github.com/gocolly/colly) - Go용 スクレイピング 프레임워크입니다
...
프로젝트를 실행하려면 유효한 プロキシ 서버가 있는지 확인하십시오. [Bright Data](https://brightdata.co.kr/)와 같은 제공업체로부터 プロキシ 서버 정보를 얻을 수 있습니다. プロキシ 서버 정보를 확보한 후, 적절한 プロキシ 구성으로 `main.go` 파일을 업데이트하십시오.
